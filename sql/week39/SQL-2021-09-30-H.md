...
Om meerdere tabellen met elkaar te linken gebruik je het `JOIN` statement. Dit geeft je alle mogelijke combinaties van de twee tabellen. `JOIN` statements zijn standaard een `INNER JOIN`. Dit hoeft niet genoemd te worden omdat het de standaard is. Je kunt het optioneel maken met blokhaken: `[INNER] JOIN`.
...
Met het `ON` key-woord kun je aangeven welke kolommen hetzelfde moeten zijn. Hier kun je [vergelijkingen](../week38/SQL-2021-09-22-H.md#Vergelijkingen) aan toevoegen. Je joint de rechter aan de linker tabel (tabel1 = links, tabel2 = rechts).
...
Je kunt een `WHERE` statement uitbreiden met een paar key-woorden. Deze werken hetzelfde als in PHP
...
# Moet je wegdenken.
```
*`1' OR 1='1` is zeer gevaarlijk* (SQL Injectie)